...
# Load required data
gas_analyzer_data       <- readRDS(file = "data/rds/gas_analyzer_20M.rds")
events_summaries_data   <- readRDS(file = "data/rds/events_summary_20M.rds")
gas_analyzer_stats_data <- readRDS(file = "data/rds/gas_analyzer_stats_20M.rds")
...
data_choice_and_fitting <- 
  ggplot(data = gas_analyzer_data) + 
    ...
    scale_shape_manual("", values = c("TRUE" = 1, "FALSE" = 16,  "ChosenValue" = 3), 
                       labels = c("TRUE" = "Deadband", "FALSE" = "included", 
...
print(data_choice_and_fitting)

# save output
psave("data-choice-and-fitting.pdf", p = data_choice_and_fitting, 
