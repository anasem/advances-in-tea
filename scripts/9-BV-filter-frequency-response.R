...
gen_data <- readRDS("data/rds/BV_frequency_response.rds")

# Create plots
# Magnitude plot
mag_plot <- 
    ggplot(data = gen_data) + 
...
phase_plot <- 
    ggplot(data = gen_data) + aes(freq, col = as.factor(q)) +
...
#
print(freq_response_plots)

# Save plot
psave("BV-filter-frequency-response.pdf", p = freq_response_plots, height = 3.7,
